...
def test_batch_only_claim_excludes_unrelated_high_confidence(database, seed_tasks):
    ...
    assert repo.claim(repo.load_session_fence(user("batch-only")), batch_code="acceptance-chosen-batch")["task_id"] == right
...
def test_airport_priority_uses_airport_evidence(database, seed_tasks):
    ...
    result = repo.claim(repo.load_session_fence(user("airport-only")), source_scene="airport")
...
def test_unknown_confidence_excludes_known_high_sources(database, seed_tasks):
    ...
    result = repo.claim(repo.load_session_fence(user("unknown-confidence")), source_confidence="unknown")
...
def test_assigned_scene_is_temporary_empty_not_nonexistent(database, seed_tasks):
    ...
    repo.claim(repo.load_session_fence(user("first-airport")), source_scene="airport")
...
def test_scope_uses_allowed_evidence_and_existing_work_resumes(database, seed_tasks):
    ...
    first = repo.claim(repo.load_session_fence(uid))
    ...
    resumed = repo.claim(repo.load_session_fence(uid), source_scene="shopping")
...
def test_same_confidence_preserves_allocation_order(database, seed_tasks):
    ...
    assert repo.claim(repo.load_session_fence(user("fifo-within-confidence")))["task_id"] == first
...
def test_twenty_users_claim_distinct_tasks(database, seed_tasks):
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        claims = list(executor.map(
            lambda uid: repo.claim(repo.load_session_fence(uid), source_scene="airport")["task_id"], users
        ))
...
def test_unauthorized_selected_scene_is_forbidden(database, seed_tasks):
    ...
    with pytest.raises(repo.ForbiddenError):
        repo.claim(repo.load_session_fence(uid), source_scene="airport")
    with pytest.raises(repo.ForbiddenError):
